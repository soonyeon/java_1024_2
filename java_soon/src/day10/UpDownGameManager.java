 ...
 */
	...
	/**
	 * 주어진 기록 정보를 이용하여 메뉴에따른 기능을 실행하는 메소드.
	...
	private static void writeRecord(Object records, int tryCount) {
		int index = 0;
		for(int i =0; i<records.length; i++) {
		...
		System.arraycopy(records, index,records,tryCount+1,records.length = index-1);
		System.out.println("이름 입력(예 : A B C) : ");
		Scanner scan = new Scanner(System.in);
