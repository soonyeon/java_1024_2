...
//				 *
			   // 메뉴 선택
				...
				if(menu == 1) {
					...
					//1. 학생 정보 추가
					// 학생 정보를 입력(학년, 반, 번호, 이름)
					System.out.println("추가할 학생정보 입력");
					...
					System.out.println("2. 학생 정보 추가 기능 구현 예정");
//				 	2. 학생 성적 추가
//				 	학생 정보를 입력(학년, 반, 번호)
					System.out.println("성적을 추가할 학생정보 입력");
					System.out.println("학년 :");
					int grade = scan.nextInt();
					System.out.println("반 :");
					int classNum = scan.nextInt();
...
//					성적 정보를 입력(과목, 학기, 중간, 기말, 수행평가)
					...
					//선택한 학생의 성적 정보들 중에서 과목 학기가 같은 성적 정보가 없으면 추가
			 		boolean res = stds[index].addScore(new Score(title, trem, midScore, finalScore, performace));
			 		if(res) {
