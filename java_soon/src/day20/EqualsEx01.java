...
public class EqualsEx01 {
	...
	public static void main(String[] args) {
		...
		list.add(t1);
		/*리스트의 contains와 indexOf는 Test의 equals를 비로 호출하는 것이 아니라
		 * objects.equals(o1, o2)를 호출한 뒤, 내부에서 o1과 o2가 같은 클래스의 객체이면
		 * Test의 equals를 호출
		 *  - 두 객체가 같은 클래스의 경우
		 *  contains() -> Obcect.equals() ->Test.qaulas()를 이용하여 true/false리턴
		 *  - 두객체가 다른 클래스의 경우
		 *  contains() -> Objects.equals() -> false 리턴
		 * 
		 */
		System.out.println("Test 객체와 Integer 객체 비교(contains) : ; "
				+list.contains((Integer)10));
		System.out.println("Test 객체와 Integer 객체 비교(indexOf) : ; "
				+list.contains((Integer)10));	
