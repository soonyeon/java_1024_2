...
-- 컴퓨터 공학부에 2022년 홍길동 교수님이 오셨다 이때 홍길동 교수님의 정보를 추가하는 작업을 해보세요
...
-- 학생 정보 추가를 위한 프로시저
-- 2022년에 고길동 학생이 컴퓨터 공학부에 입학하였다. 지도교수님은 같은과 홍길동 교수님
...
-- '컴퓨터 개론',  '월1,2' ,3, 1,2022, 1, '202216001'
insert into lecture
values(NULL, '컴퓨터 개론', '월1,2', 3, 1, 2022, 1, '202216001'),
(NULL,'자바', '화1,2,3,4', 3,1, 2022, 1, '2022160002');

delete from student where st_num = '2022160002';
...
insert into course(co_st_num, co_le_num)
	select '2022160001', le_num from lecture
		where le_name like '컴퓨터 개론' and le_term like 1 and le_years = 2022;
insert into course(co_st_num, co_le_num)
	select '2022160001', le_num from lecture
		where le_name like '자바' and le_term like 1 and le_years = 2022;
insert into course(co_st_num, co_le_num)
