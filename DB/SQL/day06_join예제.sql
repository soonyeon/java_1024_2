...
join
- 두개 이상의 테이블을 합쳐서 검색
- 이유: 알고있는 데이터와 조회하는 데이터가 서로 다른 테이블에 있고, 테이블들을 속성으로 연결할 수 있는 경우 사용
--	 테이블 1과 테이블 2가 연결
...
-- 테이블 1과 테이블 2가 연결, 테이블 2와 테이블3이 연결
	join 테이블2
		on 테이블1, 속성명1 = 테이블2, 속성명2
   join 테이블3
...
-- select me_name, product_option, or_me_id   from member
-- 	join order
--     on me_id = or_me_id;
