...
select 
	...
	속성 | 식 등..
	[from 테이블명;]
    [where 조건절] -- 조건에 맞는 결과를 조회, '=='r가 아닌 '='를, 같지 않은 경우는 '!='를 이용하여 비교
...
select * from member;
-- 	memeber 테이블에 아이디가 abc인 회원 정보를 조회
...
-- 가전 제품(제품 카테고리1번) 중 가장 싼 제품보다 가격이 높은 제품들
select * from product
	where pr_price > any (select pr_price from product where pr_pc_num = 1);
-- 1000, 5000,300 (이 숫자들중 하나라도 아래에 큰 숫자가 있으면)
-- 1000, 5000, 300, 200, 1000, 330, 220
