-- abc 회원이 반집업 맨투맨(1번 제품) 빨강을 2개 장바구니에 담았다. 이때 실행되어야 하는 쿼리를 작성하세요
...
-- select2, 'abc', po_num from product_option where po_pr_num = 1 and po_title = '빨강';

-- abc회원이 장바구니에 담긴 모든 제품을 구매. 이때 실행되어야 하는 쿼리를 작성하세요
-- 적립 포인트 제품 금액의 10%. 사용포인트는 X. 배송지는 집
-- 주문 내역에 데이터 추가
insert into 
	`order`(or_total_price, or_price, or_use_point,or_add_point,or_me_id,or_ab_num)
    value(17000*2, 17000*2, 0, 17000*2/10, 'abc', 1);
